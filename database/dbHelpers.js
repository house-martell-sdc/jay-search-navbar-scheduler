...
const queryCategories = (query, callback) => {
  ...
  const getCities = () => {
    return sequelize.query(`select cityname from cities where cityname LIKE '%${query}%'`, {type: sequelize.QueryTypes.SELECT})
  }
  
  const getCuisines = () => {
    return sequelize.query(`select cuisinename from cuisines where cuisinename LIKE '%${query}%'`, {type: sequelize.QueryTypes.SELECT})
  }

  const getRestaurants = () => {
    return sequelize.query(`select restaurantname from restaurants where restaurantname LIKE '%${query}%'`, {type: sequelize.QueryTypes.SELECT})
  }
